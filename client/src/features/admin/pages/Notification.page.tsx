...
const NotificationColumns: ColumnDef<INotification>[] = [
...

  {
    header: "Audience",
    cell: ({ row }) => {
      const { targetAudience, recipients } = row.original;

      if (targetAudience === "all") return "All Members";
      if (targetAudience === "active_members") return "Active Members";

      return `Specific (${recipients?.length ?? 0})`;
    },
  },
